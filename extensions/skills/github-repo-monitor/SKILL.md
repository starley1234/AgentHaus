...
description: Этот навык следует использовать, когда пользователь просит «следить за репозиторием GitHub», «смотреть GitHub на предмет задач или PR», «отвечать на упоминания @OpenHands в GitHub», «настроить интеграцию OpenHands с GitHub», «запускать OpenHands из GitHub».
triggers:
  - /github-monitor:poll
---
