...
description: Создание автоматизации, которая ревьюит пулл-реквесты GitHub при применении настраиваемой триггер-метки. Детерминированно опрашивает GitHub, запускает один диалог ревью OpenHands на каждое событие метки, изучает полный контекст репозитория и PR.
triggers:
  - /pr-reviewer:setup
---
