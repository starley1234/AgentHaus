...
description: Этот навык следует использовать, когда пользователь просит «следить за каналом Slack», «смотреть Slack на предмет сообщений», «создать Slack-бота, отвечающего на упоминания», «настроить интеграцию OpenHands со Slack», «запускать OpenHands из Slack».
triggers:
  - /slack-monitor:poll
---
