...
export interface StartConversationOptions {
  ...
  runtimeServicesInfo?: RuntimeServicesInfo | null;
  // When set, overrides the conversation's LLM config (used to start the chat
  // directly on a chosen LLM profile so the default model is never loaded).
  llmOverride?: SettingsRecord;
}
...
export function buildStartConversationRequest(
  ...
  if (options.llmOverride) {
    agentSettings.llm = { ...agentSettings.llm, ...options.llmOverride };
  }
