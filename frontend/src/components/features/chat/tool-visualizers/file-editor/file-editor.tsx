...
/** Collect all image URLs from an observation's content blocks. */
const imageUrlsFromContent = (
  content: Array<TextContent | ImageContent> | undefined | null,
): string[] =>
  (content ?? [])
    .filter((c) => c.type === "image")
    .flatMap((c) => (c.type === "image" ? c.image_urls : []));
 ...
 * before vs after; `create`/`view` → the file content. If the observation
 * carries images (e.g. `file_editor view` on a PNG returns an ImageContent),
 * render them as a carousel so the rendered result is visible in the chat.
 * Action card (shown while the edit is in flight): `create` → new content;
 * `str_replace` → diff of the replaced snippet; `view`/`undo_edit` → just the
 * path + range.
 */
export const fileEditorVisualizer = defineVisualizer({
  ...
  Body: function FileEditorBody({ action, observation }) {
    ...
    if (observation) {
      ...
      const imageUrls = imageUrlsFromContent(obs.content);
      ...
      if (obs.error) {
        ...
        // `file_editor view` on an image returns ImageContent blocks — show
        // them directly (the text is just "Image file ... read successfully").
        body = <ImageCarousel size="small" images={imageUrls} />;
