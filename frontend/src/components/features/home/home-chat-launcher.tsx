...
export function HomeChatLauncher() {
  ...
  const handleSubmit = (message: string) => {
    ...
    // Agent / LLM profile selected on the home page for THIS chat. Applied
    // per-chat (default untouched), then cleared.
    const {
      agentProfileId: pendingAgentProfileId,
      llmProfileName: pendingLlmProfileName,
    } = useNewChatProfilesStore.getState();

    // Launch the chat directly on the chosen LLM profile (resolved into the
    // conversation's LLM config) so the default model is never loaded. When an
    // LLM profile is chosen it takes precedence over an agent profile, because
    // the inline-agent path is what applies the LLM override.
    const useLlmProfile = !!pendingLlmProfileName;
    let variables: Parameters<typeof createConversation>[0] = {
      ...
      ...(pendingAgentProfileId && !useLlmProfile
        ? { agentProfileId: pendingAgentProfileId }
        : {}),
      ...(useLlmProfile ? { llmProfileName: pendingLlmProfileName } : {}),
    };
    ...
    void (async () => {
      try {
        ...
        // The chosen LLM profile was applied at creation (llmProfileName →
        // conversation LLM config), so the default model is never loaded and no
        // post-hoc /switch_profile is needed. Clear the pending pick.
        useNewChatProfilesStore.getState().clear();
