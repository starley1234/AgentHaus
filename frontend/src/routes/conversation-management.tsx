...
export default function ConversationManagement() {
  ...
  return <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 pb-10">
    ...
    <div className="overflow-hidden rounded-xl border border-light"><div className="flex items-center gap-3 border-b border-light px-4 py-3 text-sm"><input type="checkbox" checked={filtered.length > 0 && selected.size === filtered.length} onChange={toggleAll} aria-label="Выбрать все"/><span>{selected.size ? `Выбрано: ${selected.size}` : `Показано: ${filtered.length}`}</span></div>{loading ? <div className="p-8 text-center text-sm text-tertiary-light">Загрузка…</div> : filtered.length ? ordered.map(c => <div key={c.id} className="flex items-center gap-3 border-b border-light px-4 py-3 last:border-0"><input type="checkbox" checked={selected.has(c.id)} onChange={()=>toggle(c.id)} aria-label={`Выбрать ${c.title ?? c.id}`}/><div className="min-w-0 flex-1"><Link className="font-medium hover:underline" to={`/conversations/${c.id}`}>{c.title || "Без названия"}</Link><div className="truncate text-xs text-tertiary-light">{c.selected_repository || c.id} · {new Date(c.updated_at).toLocaleString("ru-RU")}</div></div><div className="hidden text-right text-xs text-tertiary-light sm:block"><div>↑ {formatNumber(tokens(c)?.prompt_tokens ?? 0)}</div><div>↓ {formatNumber(tokens(c)?.completion_tokens ?? 0)}</div></div></div>) : <div className="p-10 text-center text-sm text-tertiary-light">Диалоги не найдены</div>}</div>
