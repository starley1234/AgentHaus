...
export interface CreateConversationVariables {
  ...
  // Override the conversation's LLM config (used to start the chat directly on
  // a chosen LLM profile so the default model is never loaded).
  llmOverride?: Record<string, unknown>;
  // Name of an LLM profile to launch the chat on. Resolved server-side config
  // is used as the conversation's LLM (default model is never loaded).
...
export const useCreateConversation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (
      ...
      // If an LLM profile was chosen for this chat, resolve its config and use
      // it as the conversation's LLM so the chat starts directly on it (the
      // default model is never loaded).
      let resolvedLlmOverride = llmOverride;
      if (llmProfileName) {
        try {
          ...
          const config = (profile as { config?: Record<string, unknown> })
            ?.config;
          if (config) {
            resolvedLlmOverride = {
              ...config,
          ...
          // Non-fatal: fall back to the default LLM.
        }
      }
      ...
      // When an LLM profile is explicitly chosen for this chat, it takes
      // precedence: force the inline-agent path so the resolved LLM config
      // (llmOverride) is applied. The agent-profile path would otherwise be
      // used (when an active agent profile exists) and the server would ignore
      // agent_settings.llm, launching the profile's default model instead.
      if (llmProfileName) {
      ...
      const conversation =
        await AgentServerConversationService.createConversation(
          ...
          agentType,
          undefined, // sandboxId
          effectiveAgentProfileId, // agentProfileId
          resolvedAgentProfile?.agent_kind, // agentProfileKind
          resolvedLlmOverride, // llmOverride
        );
      ...
      const activeProfile = llmProfileName
        ? llmProfileName
