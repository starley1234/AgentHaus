...
export interface ChatInputLlmProfileState {
  ...
  /** Whether this picker is on the home/new-chat page (no active conversation). */
  isHomePage: boolean;
  /**
   * Switch the LLM profile for a specific chat: live via `/switch_profile`
   * when inside a conversation, or remember the choice for the next chat when
   * on the home page (applied on send). Either way the pick is scoped to one
   * chat and never changes the global default profile (that is only done in
...
export function useChatInputLlmProfileState(): ChatInputLlmProfileState {
  ...
  const setLlmProfileName = useNewChatProfilesStore((s) => s.setLlmProfileName);
  // Profile the user picked on the home page for the chat they're about to
  // start. On the home page (no active conversation) it should be shown as the
  // current selection so the pill reflects the pick immediately.
  const pendingLlmProfileName = useNewChatProfilesStore(
    (s) => s.llmProfileName,
  );
  ...
  const conversationProfile =
    stampedProfile && profiles.some((p) => p.name === stampedProfile)
      ? stampedProfile
      : null;
  // On the home page, surface the profile the user picked for the next chat;
  // otherwise fall back to the conversation/active profile.
  const homeProfileName = !conversationId ? pendingLlmProfileName : null;
  ...
  // A home pick remembers the profile for the next chat (see
  // useNewChatProfilesStore). On cloud, an account-wide activate would be the
  // org-gated endpoint a member can only 403 on; a cloud `task-…` route has no
  // real conversation id yet, so per-conversation /switch_profile would 404.
  // Either way the pill still names the active profile; only the selectable
  // rows drop out.
  ...
  const selectProfile = useCallback(
    (profileName: string) => {
      ...
      if (conversationId) {
        ...
        // Live-swap this conversation's LLM (default profile untouched).
        switchAndLog(conversationId, profileName);
        return;
      }
      // Home / new-chat page: remember the chosen LLM profile for the chat
      // you're about to start, instead of activating it account-wide or
      ...
      // Settings → LLM profiles. Clicking the currently-selected one clears it
      // back to the active default.
      setLlmProfileName(
        profileName === pendingLlmProfileName ? null : profileName,
      );
    },
    [
      canSwitchProfile,
