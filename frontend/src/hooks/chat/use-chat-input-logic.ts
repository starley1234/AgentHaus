...
export const useChatInputLogic = () => {
  ...
  // Prefill the home-page input from the "task types" quick-start tiles (or
  // any pendingPrompt) without creating a chat. Applied once, then cleared.
  const pendingPrompt = useNewChatProfilesStore((s) => s.pendingPrompt);
  const setPendingPrompt = useNewChatProfilesStore((s) => s.setPendingPrompt);
  useEffect(() => {
    if (conversationId || !pendingPrompt || !chatInputRef.current) {
      return;
    }
    const el = chatInputRef.current;
    const current = getTextContent(el).trim();
    el.focus();
    if (current) {
      el.textContent = `${current}\n${pendingPrompt}`;
    } else {
      el.textContent = pendingPrompt;
    }
  ...
  }, [conversationId, pendingPrompt, setPendingPrompt]);
