...
class LLMSummarizingCondenserSettings(CondenserSettings):
    ...
    def build_condenser(self, llm: LLM) -> LLMSummarizingCondenser | None:
        ...
        # модели, чтобы конденсер упреждающе сжимал контекст ДО переполнения
        # (не только по числу событий). Умножаем на коэффициент < 1, чтобы
        # сжимать с запасом, а не на самом пределе.
        if condenser_kwargs.get("max_tokens") is None:
            ctx = getattr(llm, "effective_max_input_tokens", None)
            if ctx:
                ratio = getattr(self, "max_tokens_ratio", None) or 0.8
                condenser_kwargs["max_tokens"] = int(ctx * ratio)
