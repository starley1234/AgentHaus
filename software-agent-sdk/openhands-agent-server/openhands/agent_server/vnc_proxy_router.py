...
async def proxy_http_request(request: Request, path: str = ""):
    ...
    Auth: checks the workspace session cookie (oh_workspace_session_key)
    since browsers cannot send custom headers with iframe <src> requests.
    Falls back to the X-Session-API-Key header for non-browser callers.
    """
    # ── Auth check (inline, avoids FastAPI dependency crash on WS scope) ──
    config = request.app.state.config
    if config.session_api_keys:
        header_key = request.headers.get("X-Session-API-Key")
        cookie_key = request.cookies.get("oh_workspace_session_key")
        if header_key not in config.session_api_keys and cookie_key not in config.session_api_keys:
